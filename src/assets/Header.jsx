...
import headerpanelpng from './images/headerpanels.png'
...
function Header(props) {
    ...
    const getTotalChars = (checkWrong = false) => { 
        let sum = 0
        for (let word of props.words) {
            if (checkWrong) {
                if (word.status === 'incorrect') {
                    if (word.hasOwnProperty('accepted') && word.accepted) {
                        sum += word.body.length
                        continue
                    } else if (word.hasOwnProperty('accepted') && !word.accepted) {
                        continue
                    }
    ...
    React.useEffect(() => { //When the game starts, Header defines times at which it should log WPM and CPM for chart data
        if (props.started) {
            ...
            props.tools.changeFinishData(
                prev => ({...prev, 
                    autocompletion: props.settings.completionKey == "auto" ? true : false, 
                    leisure: props.settings.leisure ? true : false
            }))
        } else {
    ...
    const handleSettingsToggle = async () => {
        //props.tools.changeSettings(prev => ({...prev, activated: !prev.activated}))
        if (props.settings.activated) {
            try {
                document.getElementById('settings').classList.remove('animSettings')
                document.getElementById('settings').classList.add('closeSettings')
            ...
            props.tools.changeSettings(prev => ({...prev, activated: true}))
        }
    }
    ...
    return (
        ...
        style={{backdropFilter: props.settings.theme == "dark-zero" ? 'brightness(1.3)' : props.settings.theme == 'light-colorful' && 'blur(1rem) brightness(0.7)'}}>
            <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                <img src={logoicon} style={{height: '1.7rem', marginRight: '0.5rem', pointerEvents: 'none', marginTop: '0.3rem'}} />
                <h1 className='--header-logo'>typerate</h1>
                <div className='--header-blinker'></div>
            </div>
            <div className='--header-container' style={{width: '46%', paddingRight:'1rem'}}>
                ...
                <div className='--header-container items-main'>
                    ...
                    <div style={{width:'100%', textAlign: 'center', marginTop: '1vh'}}>
                        <div style={{display: 'flex', justifyContent:'space-between', alignItems: 'center', fontStyle: 'italic', marginLeft: '-0.2rem'}}>
                            <h2 className='--header-info cwpm' style={{margin:0, fontSize: '1.3057rem'}}>{speed.wpm}</h2>
                            <h2 className='--header-info label' style={{paddingRight: '0.7rem'}}>wpm</h2>
                        </div>
                        <div style={{display: 'flex', justifyContent:'space-between', alignItems: 'flex-end', fontStyle: 'italic', marginLeft: '-0.4rem'}}>
                            <h2 className='--header-info cwpm' style={{margin:0, fontSize: '1.3057rem'}}>{speed.cpm}</h2>
                            <h2 className='--header-info label' style={{paddingRight: '0.9rem'}}>cpm</h2>
                        </div>
                    </div>
    ...
    >
        ...
        <div className='--header-off-time' style={
            props.settings.hideHeader ? ({width: `${(props.time / props.defTime) * 100}rem`, top: '0', opacity: props.started ? '1' : '0'})
            : ({width: `${(props.time / props.defTime) * 100}rem`, top: '12vh', opacity: props.started ? '1' : '0'})}>
            {
                props.settings.hideHeader &&
                <React.Fragment>
                    <h4 style={{color: 'rgba(255,255,255,0.4)', position: 'absolute', left: '0.5rem', top: '1.5vh', fontSize: '1rem'}}>typerate</h4>
                    <h4 
                    style={{position: 'absolute', left: `${(props.time / props.defTime) * 100 - 3.4}rem`, color: 'rgba(0,0,0,0.6)', fontSize: '1.2rem', top: '-3.2vh', fontWeight:'800',letterSpacing: '-0.1rem', marginRight: '-0.5rem'}}
